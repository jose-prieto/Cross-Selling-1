...
class cargaDatos:
    ...
    #Constructor
    def __init__(self, ruta, fecha, db):
        self.ruta = ruta
        self.fecha = fecha
        self.cartera = cartera_cliente_load(self.ruta, db, fecha)
    ...
    def p2p(self):
        return p2p_load(self.ruta, self.cartera.df, self.fecha)
    
    def inventario(self):
        return inventario_ajustado_load(self.ruta, self.cartera.df, self.fecha)
    
    def originacion(self):
        return originacion_load(self.ruta, self.cartera.df, self.fecha)
    ...
    def intervencion_euro(self):
        return intervencion_euro_load(self.ruta, self.cartera.df, self.fecha)
    
    def fideicomiso(self):
        return fideicomiso_load(self.ruta, self.cartera.df, self.fecha)
