...
class rrgg_empresa_load:
    ...
    def __init__(self, ruta, cartera):
        ...
        print("rrggEmpresa DRV - TDV monto total: ", self.df['monto'].sum())
        
        self.df = pd.merge(self.df, cartera, how='inner', right_on='MisCliente', left_on='mis')
    
    def to_csv(self):
        self.df.to_csv(self.rutaOrigin + '\\rchivos csv\rrgg_empresa.csv', index = False, header=True, sep='|', encoding='latin-1', quoting=csv.QUOTE_NONE)
